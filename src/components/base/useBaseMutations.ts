...
export const handleCreateBase = useCallback((name: string) => {
  return createBaseMutation.mutateAsync({ name });
}, [createBaseMutation]);

export const handleRenameBase = useCallback((name: string, baseId: string) => {
  return renameBaseMutation.mutate({ baseId, name });
}, [renameBaseMutation]);

export const handleDeleteBase = useCallback((baseId: string) => {
