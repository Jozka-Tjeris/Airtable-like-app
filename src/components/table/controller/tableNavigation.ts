  ...
  return useCallback(
    (direction: "up" | "down" | "left" | "right") => {
      ...
      const rowIndex = rows.findIndex(
        (r) => (r.internalId ?? r.id) === activeCell.rowId,
      );
      const colIndex = columns.findIndex(
        (c) => (c.internalId ?? c.id) === activeCell.columnId,
      );
      ...
      switch (direction) {
        case "up":
          ...
          break;
        case "down":
          nextRowIdx = Math.min(rows.length - 1, rowIndex + 1);
          break;
        case "left":
          nextColIdx = Math.max(0, colIndex - 1);
          break;
        case "right":
          nextColIdx = Math.min(columns.length - 1, colIndex + 1);
          break;
      }
